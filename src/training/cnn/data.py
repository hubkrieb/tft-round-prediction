...
class TFTBoardDataModule(L.LightningDataModule):
    ...
    def setup(self, stage: str | None = None) -> None:
        ...
        ascending (:func:`src.training.cnn.transform.sort_features_by_timestamp` runs as
        ...
        if not ts_path.exists():
            raise FileNotFoundError(
                ...
                "src.training.cnn.transform.extract_tensors."
            )
        ...
        if timestamps.size > 1 and not bool(np.all(timestamps[:-1] <= timestamps[1:])):
            raise ValueError(
                ...
                "Run src.training.cnn.transform.sort_features_by_timestamp(feature_path)."
            )
