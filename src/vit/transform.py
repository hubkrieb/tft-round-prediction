...
# Build Trait Vocab
# Sort traits to ensure deterministic ID assignment
sorted_traits = sorted(TRAITS.items())
TRAIT_VOCAB_LIST = []
for trait, bps in sorted_traits:
    for bp in sorted(bps):
        TRAIT_VOCAB_LIST.append(f"{trait}_{bp}")

TRAIT_VOCAB_MAP = {t: i + 1 for i, t in enumerate(TRAIT_VOCAB_LIST)}
...
def extract_traits_ids(team_data: pl.DataFrame, team_name: str) -> pl.DataFrame:
    ...
    trait_map_df = pl.DataFrame(
        [
            ...
            for t, i in TRAIT_VOCAB_MAP.items()
        ]
    )
